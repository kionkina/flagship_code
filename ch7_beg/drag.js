...
var ans_array =   ['臺', '對','剛','漢','後','開','寶','長','壞', '歡'];
...
for (var i=0; i<term_array.length; i++){
	...
	terms.innerHTML += "<tr>";
	terms.innerHTML += "<td style='text-align: right'  width='20px'><a id='incorrect_" + i + "' class='mark_ex6' href='#'>&#10008;</a><a  id='correct_" + i  + "' class='mark_correct_ex6' href='#'>&#10004;</a></td><td> <div class='e5_width font_kai'>" + ans_array[i] + "</div></td></tr>"; 
  ...
  $('#DROP').html('');
    	...
    	for ( var i=0; i<correctness.length; i++ ) {
			$('<div> </div>').data( 'term', text[i] ).attr('id',i).data('holding', false).appendTo( '#cardSlots' ).droppable( {
			accept: '#cardPile div',
			hoverClass: 'hovered',
			drop: handleCardDrop
    } );
